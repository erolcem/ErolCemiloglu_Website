...
def get_db_projects():
    try:
        with engine.connect() as conn:
            result = conn.execute(text("SELECT * FROM projects WHERE is_public = true ORDER BY custom_order DESC, id DESC"))
...
def get_db_skills():
    try:
        with engine.connect() as conn:
            # Fetch all skills
            result = conn.execute(text("SELECT * FROM skills WHERE is_public = true ORDER BY id ASC"))
